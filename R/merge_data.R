...
#'
#' @return List containing merged data frames.
#'
#' @export
#' @importFrom readr read_csv write_csv
#' @importFrom dplyr select rename mutate across full_join distinct arrange
...
merge_mass_intensity <- function(dir_path, output_mass_intensity, output_mass_elements, output_meta_file = NULL) {
    ...
    install_dependencies <- function() {
        required_packages <- c("readr", "dplyr", "tools", "tidyr")
    ...
    # Extract actual sample IDs from the merged data
    actual_sample_ids <- setdiff(colnames(merged_mass_intensity), "Mass")
    ...
    # Return list containing all merged data
    return(list(
        mass_intensity_data = merged_mass_intensity,
        mass_elements_data = merged_mass_elements,
        metadata = emeta,
        sample_ids = actual_sample_ids
    ))
}

#' Merge Data Based on Molecular Formula with Filtering
#'
#' @param dir_path The directory containing CSV files.
...
#' @export
#' @importFrom readr read_csv write_csv
#' @importFrom dplyr select rename mutate across full_join distinct arrange group_by ungroup slice
#' @importFrom tidyr replace_na
#' @importFrom tools file_path_sans_ext
merge_molform_intensity <- function(dir_path, output_molform_intensity, output_molform_elements, 
    ...
    for (file in file_list) {
        ...
        if (any(duplicated(df$MolForm))) {
            if ("Error" %in% colnames(df)) {
                ...
                # If no Error column, just remove duplicates by keeping the first occurrence
                df <- df %>%
    ...
    # Extract actual sample IDs from the merged data
    actual_sample_ids <- setdiff(colnames(merged_molform_intensity), "MolForm")
    ...
    # Return list containing all merged data
    return(list(
        molform_intensity_data = merged_molform_intensity,
        molform_elements_data = merged_molform_elements,
        metadata = emeta,
        sample_ids = actual_sample_ids,
