...
batch_car_analysis <- function(
  ...
  # ========== 6. Helper Function: Query Top50 from Database ==========
  ...
  # ========== 7. Phase 1: Generate All Databases ==========
  if (verbose) {
    cat("\n")
    cat(paste0(rep("=", 62), collapse = ""), "\n")
  ...
  # ========== 8. Phase 2: Query All Databases for Top50 ==========
  if (verbose) {
    cat(paste0(rep("=", 62), collapse = ""), "\n")
    cat("🔍 Phase 2: Query Top50 Matches\n")
    cat(paste0(rep("=", 62), collapse = ""), "\n\n")
  }
  ...
  # ========== 9. Phase 3: Merge Results and Generate Output ==========
  if (verbose) {
    cat(paste0(rep("=", 62), collapse = ""), "\n")
    cat("📊 Phase 3: Generate Summary Table\n")
    cat(paste0(rep("=", 62), collapse = ""), "\n\n")
  }
  ...
  wide_table$Total_Count <- rowSums(wide_table[, -1])
  wide_table$Detection_Rate <- round(
    rowSums(wide_table[, -c(1, ncol(wide_table))] > 0) / (ncol(wide_table) - 2) * 100, 2
  )
